...
void ColumnHelper::WriteColumns(idx_t start_index, idx_t start_col, idx_t end_col, DataChunk &output) {
	for (idx_t i = start_col; i < end_col; i++) {
		...
		case LogicalTypeId::DECIMAL:
			numeric_precision = Value::INTEGER(static_cast<int32_t>(DecimalType::GetWidth(type)));
			numeric_scale = Value::INTEGER(static_cast<int32_t>(DecimalType::GetScale(type)));
